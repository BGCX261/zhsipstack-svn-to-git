// Accept.cpp: implementation of the CAccept class.
//
//////////////////////////////////////////////////////////////////////
...
int CAccept::Parse( const char *hvalue )
{
	char* subtype;
...
int CAccept::Clone(CAccept** dest)
{
	int i;
	CAccept* ct;
...
int CAccept::ToStr(char** dest)
{
	...
	*dest = NULL;
	...
	if (this->subtype != NULL)
		len += strlen(this->subtype);
	
	if (len == 0)
	{
		/* Empty header ! */
		buf = (char *) mm_malloc(2);
		buf[0] = ' ';
		buf[1] = '\0';
		*dest = buf;
		return 0;
